...
font:{regex:/font\:(.*)/,output:function(val){return this.regex.exec(val)[1]}},auto:{regex:/^auto$/}};for(var flag in app.flags){if(!app.flags.hasOwnProperty(flag))continue;app.flags[flag].match=function(val){return val.match(this.regex)}}app.add_theme=function(name,theme){name!=null&&theme!=null&&(settings.themes[name]=theme);return app};app.add_image=function(src,el){var node=selector(el);if(node.length)for(var i=0,l=node.length;i<l;i++){var img=document.createElement("img");img.setAttribute("data-src",
src);node[i].appendChild(img)}return app};app.run=function(o){var options=extend(settings,o),images=[],imageNodes=[],bgnodes=[];if(typeof options.images=="string")imageNodes=selector(options.images);else if(window.NodeList&&options.images instanceof window.NodeList)imageNodes=options.images;else if(window.Node&&options.images instanceof window.Node)imageNodes=[options.images];if(typeof options.bgnodes=="string")bgnodes=selector(options.bgnodes);else if(window.NodeList&&options.elements instanceof
window.NodeList)bgnodes=options.bgnodes;else if(window.Node&&options.bgnodes instanceof window.Node)bgnodes=[options.bgnodes];preempted=true;for(i=0,l=imageNodes.length;i<l;i++)images.push(imageNodes[i]);var holdercss=document.getElementById("holderjs-style");if(!holdercss){holdercss=document.createElement("style");holdercss.setAttribute("id","holderjs-style");holdercss.type="text/css";document.getElementsByTagName("head")[0].appendChild(holdercss)}if(!options.nocss)if(holdercss.styleSheet)holdercss.styleSheet.cssText+=
options.stylesheet;else holdercss.appendChild(document.createTextNode(options.stylesheet));var cssregex=new RegExp(options.domain+'/(.*?)"?\\)');for(var l=bgnodes.length,i=0;i<l;i++){var src=window.getComputedStyle(bgnodes[i],null).getPropertyValue("background-image");var flags=src.match(cssregex);var bgsrc=bgnodes[i].getAttribute("data-background-src");if(flags){var holder=parse_flags(flags[1].split("/"),options);if(holder)render("background",bgnodes[i],holder,src)}else if(bgsrc!=null){var holder=
parse_flags(bgsrc.substr(bgsrc.lastIndexOf(options.domain)+options.domain.length+1).split("/"),options);if(holder)render("background",bgnodes[i],holder,src)}}for(l=images.length,i=0;i<l;i++){var attr_src=attr_data_src=src=null;try{attr_src=images[i].getAttribute("src");attr_datasrc=images[i].getAttribute("data-src")}catch(e){}if(attr_datasrc==null&&!!attr_src&&attr_src.indexOf(options.domain)>=0)src=attr_src;else if(!!attr_datasrc&&attr_datasrc.indexOf(options.domain)>=0)src=attr_datasrc;if(src){var holder=
parse_flags(src.substr(src.lastIndexOf(options.domain)+options.domain.length+1).split("/"),options);if(holder)if(holder.fluid)render("fluid",images[i],holder,src);else render("image",images[i],holder,src)}}return app};contentLoaded(win,function(){if(window.addEventListener){window.addEventListener("resize",fluid_update,false);window.addEventListener("orientationchange",fluid_update,false)}else window.attachEvent("onresize",fluid_update);preempted||app.run()});if(typeof define==="function"&&define.amd)define("Holder",
[],function(){return app})})(Holder,window);